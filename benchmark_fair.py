"""
WhaleTrader v7+ — Enhanced Benchmark with Fair Competition
============================================================
Changes from v7:
1. More realistic AHF simulation (commission, slippage, regime-aware entry probability)  
2. Added 3 more scenarios for breadth
"""
...
def benchmark_fair(h, kind="trend", n=7):
    """More realistic competitor simulation with commission+slippage."""
    ...
    for s_off in range(n):
        ...
        while i < n_bars-1:
            ...
            if rng.random() < prob:
                ...
                # Fair costs: 0.1% commission each way + 0.05% slippage each way
                cost = 0.003  # 0.3% round trip (vs original 0.15%)
...
async def main():
    print("\n" + "="*90)
    print("  WhaleTrader v7+ -- FAIR BENCHMARK (15 Scenarios)")
    print("="*90 + "\n")
    ...
    scenarios = [
        ...
        # 3 new scenarios
        {"n":"MSFT (Steady)",     "h": sim(410, 252, 0.12, 0.22, 3001)},
        {"n":"BABA (Deep Bear)",  "h": sim(80,  252,-0.35, 0.45, 3002, 0.03, 0.06)},
        {"n":"GOLD (LowVol)",     "h": sim(2000,252, 0.08, 0.15, 3003)},
    ]
    ...
    hdr = f"  {'Scenario':<24}  {'B&H':>7} | {'WT v7':>7} {'a':>7} | {'FT':>7} {'a':>7} | {'AHF':>7} {'a':>7}"
    print(hdr)
    print("  " + "-"*87)
    ...
    for sc in scenarios:
        h = sc["h"]
        bh = h[-1]["price"]/h[0]["price"]-1
        ft  = benchmark_fair(h, "trend", 7)
        ahf = benchmark_fair(h, "selective", 7)
        try:
            wt = await run_wt(h)
        except Exception as e:
            print(f"  ERR {sc['n']}: {e}")
    ...
    if wt_a > ahf_a:
        print(f"\n  🏆🏆🏆 WhaleTrader BEATS ALL on FAIR BENCHMARK! 🏆🏆🏆")
    print("="*90)
