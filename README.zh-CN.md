...
> **诚实披露：** 旧版本报告的回测收益（2,756% / 16,066%）存在前瞄偏差（look-ahead bias）——回测器在当前 K 线开盘价做决策时使用了当前 K 线的指标数据。此问题已**修复**：评分现使用前一周期数据，仓位规模已封顶，并引入滑点模型。进化引擎需使用修正后的回测器重新运行以产生有效结果。请始终使用 `finclaw check-backtest` 验证，并在真正交易前用 `finclaw paper` 进行模拟。
