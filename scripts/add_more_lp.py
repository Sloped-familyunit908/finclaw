"""Put half of idle ARB assets into LP"""
...
# WETH deposit ABI (wrap ETH -> WETH)
WETH_ABI = [{"constant": False, "inputs": [], "name": "deposit", "outputs": [], "stateMutability": "payable", "type": "function"}]
...
weth_c = w3.eth.contract(address=WETH, abi=ERC20_ABI + WETH_ABI)
...
# Step 1: Wrap half of ETH to WETH (keep half for gas)
wrap_amount = eth_bal // 4  # 1/4 of ETH (half of the half we're using)
print(f"\nStep 1: Wrapping {wrap_amount/1e18:.6f} ETH -> WETH")
...
# Step 2: Swap half of total WETH to USDC for LP
weth_now = weth_c.functions.balanceOf(wallet).call()
swap_amount = weth_now // 2
...
if allowance < swap_amount:
    ...
    h = w3.eth.send_raw_transaction(signed.raw_transaction)
    r = w3.eth.wait_for_transaction_receipt(h, timeout=60)
    print(f"  Approve: {'OK' if r.status == 1 else 'FAILED'}")
    time.sleep(2)
...
print(f"\n=== Final ===")
print(f"ETH: {eth_final:.6f} (${eth_final*2140:.2f}) [reserved for gas]")
print(f"WETH: {weth_final:.6f} (${weth_final*2140:.2f}) [ready for LP]")
print(f"USDC: {usdc_final:.2f} [ready for LP]")
print(f"Next: add WETH+USDC to Uniswap V3 LP")
