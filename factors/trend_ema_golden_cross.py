"""
Factor: trend_ema_golden_cross
Description: EMA(5) crosses above EMA(20) — bullish golden cross
Category: trend_following
"""

FACTOR_NAME = "trend_ema_golden_cross"
FACTOR_DESC = "EMA(5) crosses above EMA(20) — bullish trend signal"
...
def compute(closes, highs, lows, volumes, idx):
    """EMA(5) crosses above EMA(20). Score based on recency and strength."""
    if idx < 21:
        return 0.5
    ...
    # Check for golden cross (EMA5 crosses above EMA20)
    crossed_today = ema5_prev <= ema20_prev and ema5_now > ema20_now

    if crossed_today:
        # Fresh cross — high score
        if ema20_now > 0:
            strength = (ema5_now - ema20_now) / ema20_now
    ...
    if ema5_now > ema20_now:
        # EMA5 above EMA20 — bullish, but how recent was the cross?
        for lookback in range(1, 6):
            ...
            if e5 <= e20:
                # Cross was `lookback` days ago
                recency = 1.0 - lookback / 5.0
                score = 0.65 + 0.2 * recency
                return max(0.0, min(1.0, score))

        # EMA5 above EMA20 but cross was >5 days ago
        return 0.6

    return 0.5  # EMA5 below EMA20
