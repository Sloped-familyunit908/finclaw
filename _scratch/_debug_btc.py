...
async def main():
    h = await fetch_crypto("BTC", 365)
    bh = h[-1]["price"] / h[0]["price"] - 1
    bt = BacktesterV7(initial_capital=10000)
    r = await bt.run("BTC", "v7", h)
    alpha = r.total_return - bh
    print(f"BTC: B&H={bh:+.1%}  ret={r.total_return:+.1%}  alpha={alpha:+.1%}")
