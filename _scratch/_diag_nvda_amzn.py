"""Diagnose NVDA, AMZN, Moutai v7 trade details"""
...
async def main():
    test_cases = [
        ...
        ("AMZN (Bull2)",     sim(180, 252, 0.30, 0.35, 1628)),
        ("Moutai (Sideways)",sim(1650,252, 0.05, 0.30, 2001, 0.03, 0.06)),
    ]
    for name, h in test_cases:
        bh = h[-1]["price"] / h[0]["price"] - 1
        # Print price path summary
        prices = [b["price"] for b in h]
        peak_idx = prices.index(max(prices))
        print(f"\n{'='*70}")
        print(f"  {name}  B&H={bh:+.1%}  peak=${max(prices):.1f}@day{peak_idx}  end=${prices[-1]:.1f}")
        print(f"  first10: ${prices[20]:.1f} -> ${prices[30]:.1f} -> ${prices[40]:.1f}")
        print(f"{'='*70}")
        for label, BT in [("v6", BacktesterV5), ("v7", BacktesterV7)]:
            ...
            print(f"  {label}: ret={r.total_return:+.1%} alpha={alpha:+.1%} "
