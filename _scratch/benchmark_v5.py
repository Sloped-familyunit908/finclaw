"""
WhaleTrader v5 — BENCHMARK (Aggressive Trend Rider)
=====================================================
Tests SignalEngineV5 + BacktesterV4 against competitors.
"""
...
async def run_wt(h):
    ...
    r = await bt.run("TEST","WhaleTrader v5",h)
    bh = h[-1]["price"]/h[0]["price"]-1
...
async def main():
    print("\n" + "="*90)
    print("  WhaleTrader v5 — BENCHMARK (Aggressive Trend Rider)")
    print("="*90 + "\n")
    ...
    print(f"  {'Scenario':<24}  {'B&H':>7} | {'WT v5':>7} {'a':>7} | {'FT':>7} {'a':>7} | {'AHF':>7} {'a':>7}")
    print("  " + "-"*87)
    ...
    print("  " + "-"*70)
    print(f"  >> WhaleTrader v5      {wt_a:>+9.2%} {wt_dd:>10.2%}   {wt_wins_ft}/{wt_wins_ahf}/{wt_wins_both} of {N}")
    print(f"     freqtrade           {ft_a:>+9.2%} {ft_dd:>10.2%}")
    print(f"     ai-hedge-fund       {ahf_a:>+9.2%} {ahf_dd:>10.2%}")

    print(f"\n  WT vs freqtrade:     {wt_wins_ft}/{N} ({wt_wins_ft/N:.0%})")
    print(f"  WT vs ai-hedge-fund: {wt_wins_ahf}/{N} ({wt_wins_ahf/N:.0%})")
    print(f"  WT beats BOTH:       {wt_wins_both}/{N} ({wt_wins_both/N:.0%})")

    if wt_a > ahf_a:
        print(f"\n  [WINNER] WhaleTrader v5 BEATS ALL! Gap: +{wt_a-ft_a:.1%} vs FT, +{wt_a-ahf_a:.1%} vs AHF")
    elif wt_a > ft_a:
        print(f"\n  [GOOD] Beats freqtrade (+{wt_a-ft_a:.1%}). Gap to AHF: {wt_a-ahf_a:+.1%}")
    else:
        print(f"\n  [WIP] Still iterating. WT-FT={wt_a-ft_a:+.1%}, WT-AHF={wt_a-ahf_a:+.1%}")
    print("="*90)
